...
def picture(height, width,bk_path):
    """
        Create a background with a picture
    """
    # script_path = os.path.split(os.path.realpath(__file__))[0]
    # # pictures = os.listdir(os.path.join(script_path, "pictures"))
    # pictures = os.listdir(os.path.join(script_path, "bk_picture/pictures"))
   
    pictures = os.listdir(bk_path)

    if len(pictures) > 0:
        # pic = Image.open(os.path.join(script_path, "pictures", pictures[rnd.randint(0, len(pictures) - 1)]))
        # pic = Image.open(os.path.join(script_path, "bk_picture/pictures", pictures[rnd.randint(0, len(pictures) - 1)]))
        pic = Image.open(os.path.join(bk_path, pictures[rnd.randint(0, len(pictures) - 1)]))
