...
def _apply_func_distorsion(image, vertical, horizontal, max_offset, func):
    ...
    if vertical:
        ...
        for i, o in enumerate(vertical_offsets):
            ...
            new_img_arr[
                max_offset + o : column_height + max_offset + o, column_pos] = img_arr[:, i]
    ...
    if horizontal:
        ...
        for i, o in enumerate(horizontal_offsets):
            if vertical:
                ...
                new_img_arr[
                    i, max_offset + o : row_width + max_offset + o] = img_arr[i, :]
    ...
    #     ).convert("RGBA")
    # )
    return np.uint8(new_img_arr_copy if horizontal and vertical else new_img_arr)
...
def sin(image, vertical=False, horizontal=False):
    ...
    max_offset = int(image.shape[0] ** 0.5)
...
def random(image, vertical=False, horizontal=False):
    ...
    max_offset = int(image.shape[0] ** 0.4)
